...
export class ServiceRequestManager {
  ...
  async createRequest(
    ...
    // Immediately show "Request Accepted" status
    toast({
      title: "Request Accepted",
      description: "Your request has been accepted by our team."
    });
    
    // Start finding employee after a brief moment
    setTimeout(() => {
  ...
  async declineQuote(): Promise<void> {
    ...
    if (this.currentRequest.declineCount === 1 && !this.currentRequest.hasReceivedRevision) {
      ...
      // Second decline (Final Decline) - blacklist employee and find new one
  ...
  private async blacklistCurrentEmployeeAndFindNew(): Promise<void> {
    ...
    try {
      ...
      // Reset employee assignment and decline count for new employee
      this.currentRequest.assignedEmployee = null;
      this.currentRequest.currentQuote = null;
      this.currentRequest.declineCount = 0; // Reset decline count for new employee
  ...
  private simulateServiceProgress(): void {
    ...
    // Employee reaches user location after 5 seconds, then completes service
    setTimeout(() => {
